...
extension DetailScreen {
    class ViewModel: ObservableObject {
        ...
        @Published var selectedPause: WorkDayPause?
        @Published var model: WorkDay
        ...
        var disableAddPause: Bool {
            return selectedPause == nil &&  model.pause.count == 4
        }
...
//                    persistenceController.save()
        ...
        private func calculatePauseInSeconds() -> Int {
            ...
            if model.pause.count != 0 {
                
                // Iterate over each pause in the array
                for pause in model.pause {
                    
                    // Calculate the duration of the current pause
                    let pauseTime = pause.finishPause.timeIntervalSince(pause.startPause)
